...
FROM
    ...
    WHERE
        t1.id < t2.id AND t2.pos = 'PG'
            AND t1.pos = 'PG'
            AND t1.proj_score > 20
            AND t2.proj_score > 20) AS pgs
    CROSS JOIN (SELECT 
        t1.id AS SG1,
            t2.id AS SG2,
    ...
    WHERE
        players_v.pos = 'C'
            AND players_v.proj_score > 20) AS cs)
WHERE
    PGS_PTS + SGS_PTS + PFS_PTS + SFS_PTS + CS_PTS > 150
